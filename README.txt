...
1. **Create the Database**:
    ...
    - Execute the following SQL script to create the `AgriDB` database:
      ...
      ```
    - Or Use this method using the AngriDB Bak file: 

Open SSMS on the User's Machine:

The users should open SQL Server Management Studio and connect to their SQL Server instance.
...
In the appsettings.json file of the project, users need to update the connection string to point to their local SQL Server instance
{
  "ConnectionStrings": {
...
Replace YOUR_SERVER_NAME with the name of their SQL Server instance.
...
3. **Verify if tables have been added to the database**:
     -```Run these queries on a sql queries to verify databse table creation 
USE AgriDB;
SELECT * FROM Farmers;
